...
export async function register(email, password) {         // !!! ВСЯКА НОВА ЗАДАЧА може да има други входни ПАРАМЕТРИ
    const result = await post('/users/register', { email, password });        // The service expects a body with the following shape:  { username, email, password, gender }
    console.log(result);
